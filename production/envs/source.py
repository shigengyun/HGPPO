        ...
        while True:
            if self.parameters['SOURCE_ORDER_GENERATION_TYPE'] == "ALWAYS_FILL_UP":
                if len(self.buffer_out) >= self.capacity:
                    self.idle.succeed()
                    break
                yield self.env.timeout(self.parameters['EPSILON'] * 25)

                if Source.counter_order >= self.parameters['NUM_ORDERS'] - 1:
                    break
            elif self.parameters['SOURCE_ORDER_GENERATION_TYPE'] == "MEAN_ARRIVAL_TIME":
                base_time = self.time_calc.time_to_order_generation(self, self.statistics, self.parameters)
                yield self.env.timeout(base_time * 3.5)
            extra_delay = random.uniform(1.0, 2.5)
            yield self.env.timeout(extra_delay)
            order_type = random.choice(['P1', 'P2', 'P3'])
            prod_steps, variant = self.time_calc.create_intermediate_production_steps_and_variant(
                ...
                at_resource=self, create_type=order_type)
